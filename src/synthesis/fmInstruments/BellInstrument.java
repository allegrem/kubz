...
public class BellInstrument extends FmInstrumentNParams {
	...
	private final float STEPS = 1000; // lets define the parameters on a scale
										// of 1 to 1000
	...
	/**
	 * Constructs a bell with fm, attack and decay values as controllable
	 * parameters.
	 */
	public BellInstrument() {
		...
		FixedADSR env = new FixedADSR(a.getValue() / STEPS, d.getValue()
				/ STEPS, 0.0f, 0.0f, 3f);
		Gain fp = new Gain((float) (1 / Math.sqrt(2)), fm);
		try {
			env.plugin(new Constant((float) 100));
		} catch (TooManyInputsException e) {
			e.printStackTrace();
		}
		/*SineOscillator osc1 = new SineOscillator(fm, new Constant(
				(float) 190));*/
