...
public class CLCamera {
	...
	native static int CLEyeCreateCamera(int cameraIndex, int mode,
			int resolution, int framerate);
	...
	native static boolean CLEyeSetCameraParameter(int cameraInstance,
			int param, int val);

	native static int CLEyeGetCameraParameter(int cameraInstance, int param);

	native static boolean CLEyeCameraGetFrame(int cameraInstance,
			int[] imgData, int waitTimeout);
...
//	private PApplet parent;
	...
	public static void loadLibrary(String libraryPath) {
		...
		try {
			...
			System.out
					.println("(3) Could not find the CLEyeMulticam.dll (Custom Path)");
		}
	}
	...
	public boolean createCamera(int cameraIndex, int mode, int resolution,
			int framerate) {
		cameraInstance = CLEyeCreateCamera(cameraIndex, mode, resolution,
				framerate);
